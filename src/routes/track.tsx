...
import { Search, PackageSearch, CheckCircle2, Clock, Loader2, XCircle, Hourglass } from "lucide-react";
...
export const Route = createFileRoute("/track")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Track Your Commission — Paras Arts" },
      { property: "og:description", content: "Live status for your Paras Arts pencil-art commission." },
...
const STAGES = ["Pending", "Accepted", "In Progress", "Completed"] as const;

const META: Record<string, { icon: typeof Clock; note: string }> = {
  Pending: { icon: Hourglass, note: "We've received your enquiry and are preparing your quote." },
  Accepted: { icon: CheckCircle2, note: "Your commission is confirmed and queued in the studio." },
  "In Progress": { icon: Loader2, note: "Paras is drawing your piece — progress photos are shared." },
  Completed: { icon: CheckCircle2, note: "Finished, sealed and dispatched with tracking." },
  Cancelled: { icon: XCircle, note: "This commission was cancelled. Contact the studio to restart." },
};
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const { data } = await api.get("/orders/track", {
        params: { email: cleanEmail, id: cleanId },
      });
      setOrder(data);
    } catch (err) {
                ...
                return (
                  <li
                    ...
                    className={`rounded-2xl border p-5 transition-colors ${
                      done ? "border-gold/50 bg-gold/[0.06]" : "border-white/10 bg-white/[0.02]"
                    }`}
                  >
